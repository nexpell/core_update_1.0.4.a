...
class Template
{
    ...
    public function renderPagination(string $baseUrl, int $currentPage, int $totalPages, string $pageParam = 'page'): string
    {
        ...
        $buildPageUrl = static function (string $url, int $page) use ($pageParam): string {
            if (strpos($url, '?') === false) {
                $url = preg_replace('#/' . preg_quote($pageParam, '#') . '/[0-9]+/?$#', '', $url);
            }
            ...
            return rtrim($url, '/') . '/' . rawurlencode($pageParam) . '/' . $page;
        };

        global $languageService;

        $previousLabel = (isset($languageService) && method_exists($languageService, 'get'))
            ? (string)$languageService->get('previous')
            : 'Previous';
        $nextLabel = (isset($languageService) && method_exists($languageService, 'get'))
            ? (string)$languageService->get('next')
            : 'Next';

        $prevHref = $currentPage > 1 ? htmlspecialchars($buildPageUrl($baseUrl, $currentPage - 1), ENT_QUOTES, 'UTF-8') : '#';
        $html .= '<li class="page-item' . $prevDisabled . '">';
        $html .= '<a class="page-link" href="' . $prevHref . '">' . htmlspecialchars($previousLabel, ENT_QUOTES, 'UTF-8') . '</a>';
        $html .= '</li>';
        ...
        for ($i = 1; $i <= $totalPages; $i++) {
            ...
            $html .= '<a class="page-link" href="' . htmlspecialchars($buildPageUrl($baseUrl, $i), ENT_QUOTES, 'UTF-8') . '">' . $i . '</a>';
            $html .= '</li>';
        }

        // Weiter-Button
        $nextDisabled = ($currentPage >= $totalPages) ? ' disabled' : '';
        $nextHref = $currentPage < $totalPages ? htmlspecialchars($buildPageUrl($baseUrl, $currentPage + 1), ENT_QUOTES, 'UTF-8') : '#';
        $html .= '<li class="page-item' . $nextDisabled . '">';
        $html .= '<a class="page-link" href="' . $nextHref . '">' . htmlspecialchars($nextLabel, ENT_QUOTES, 'UTF-8') . '</a>';
        $html .= '</li>';
