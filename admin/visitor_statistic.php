...
foreach (array_slice($top_countries_grouped, 0, 6, true) as $country_code => $country_visitors) {
    $top_countries[] = [
        'country_code' => $country_code,
        'visitors' => $country_visitors,
    ];
}
...
$chartPrimaryColor = !empty($uiPrimaryColor) ? (string)$uiPrimaryColor : '#fe821d';
...
window.__visitorTraffic = {
    ...
    maxOnline: <?= nx_json_script($maxonline_values) ?>
};
</script>
<script>
document.addEventListener('DOMContentLoaded', function () {
    if (!window.ApexCharts || !window.__visitorTraffic) return;

    const visitorsEl = document.querySelector('#visitorsChart');
    if (!visitorsEl) return;

    const traffic = window.__visitorTraffic;
    const visitsLabel = <?= nx_json_script($visitsLabel) ?>;
    const safeVisitorsData = Array.isArray(traffic.visitors)
        ? traffic.visitors.map((value) => Number.isFinite(Number(value)) ? Number(value) : 0)
        : [];
    const safeMaxOnlineData = Array.isArray(traffic.maxOnline)
        ? traffic.maxOnline.map((value) => Number.isFinite(Number(value)) ? Number(value) : 0)
        : [];
    ...
    const primary = cssVar('--ac-primary', <?= nx_json_script($chartPrimaryColor) ?>);
    ...
    const visitorsOptions = {
        ...
        series: [
            { name: visitsLabel, data: safeVisitorsData },
            { name: 'MaxOnline', data: safeMaxOnlineData }
        ],
        xaxis: {
            categories: Array.isArray(traffic.labels) ? traffic.labels : [],
        ...
        yaxis: [
            {
                min: 0,
                title: { text: 'Hits' },
                labels: { formatter: (val) => Number.isFinite(Number(val)) ? Math.round(Number(val)) : '' },
                axisBorder: { show: false },
                axisTicks: { show: false }
            },
            {
    ...
    try {
        ...
        visitorsEl.innerHTML = '<div class="text-danger small">Chart konnte nicht geladen werden.</div>';
        console.error('visitorsChart render failed', error);
    }
});
</script>
<script>
