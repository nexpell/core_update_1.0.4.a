...
$facebook_url  = htmlspecialchars(normalize_external_url((string)($user_socials['facebook'] ?? '')));
$twitter_url   = htmlspecialchars(normalize_external_url((string)($user_socials['twitter'] ?? '')));
$instagram_url = htmlspecialchars(normalize_external_url((string)($user_socials['instagram'] ?? '')));
$website_url   = htmlspecialchars(normalize_external_url((string)($user_socials['website'] ?? '')));
$github_url    = htmlspecialchars(normalize_external_url((string)($user_socials['github'] ?? '')));
...
// 1. Profil-User bestimmen: erst GET, dann SESSION
...
if ($viewUserID > 0) {
    $sql = "
        SELECT
            ...
            (
                ...
                WHERE us.userID = ?
            ) AS session_last_seen
        FROM users u
        WHERE u.userID = ?
    ";
    $stmt = $_database->prepare($sql);
    $stmt->bind_param("ii", $viewUserID, $viewUserID);
    $stmt->execute();
    ...
    if ($user) {
        ...
        $last_activity_timestamp = !empty($user['last_activity']) ? strtotime((string)$user['last_activity']) : 0;
        $session_last_seen = !empty($user['session_last_seen']) ? (int)$user['session_last_seen'] : 0;
        $session_is_open = ((int)($user['is_online'] ?? 0) === 1)
            && $login_timestamp > 0
            && $session_last_seen >= (time() - $idleLimitSeconds);

        $is_online = $session_is_open;
        $is_idle = $session_is_open
            && $last_activity_timestamp > 0
            && $last_activity_timestamp < (time() - $idleLimitSeconds);
...
$data_array = [
    ...
    <tr><td>' . $languageService->get('current_session') . ':</td><td>' . ($is_online
            ? '<span id="current-session">' . formatTime($current_session_seconds) . '</span>' .
              ($is_idle ? ' <small class="text-muted">(' . $languageService->get('user_inactive') . ')</small>' : '')
