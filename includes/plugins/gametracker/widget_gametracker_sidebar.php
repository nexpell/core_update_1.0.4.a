...
echo '<strong><i class="bi bi-controller"></i> ' . $languageService->get('title_gameserver') . '</strong>';
echo '</div>';
...
if (!function_exists('stripColorCodes')) {
...
if (!function_exists('colorizeName')) {
...
foreach ($queryList as $server) {
    ...
    echo '<li class="list-group-item text-muted">' . $languageService->get('info_no_active_servers') . '</li>';
}

echo '</ul>';
echo '<div class="card-footer text-center p-2">';
echo '<a href="' . SeoUrlHandler::convertToSeoUrl('index.php?site=gametracker') . '" 
    class="btn btn-sm btn-outline-primary w-100">' . $languageService->get('btn_all_servers') . '</a>';
