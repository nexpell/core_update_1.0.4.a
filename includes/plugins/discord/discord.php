...
<?php if (!empty($serverID)): ?>
  <div class="discord-embed-shell discord-embed-shell--page" id="discord-card" style="display:none;">
    <div class="card shadow mb-4 widget-discord discord-panel">
      ...
      <div class="card-body p-0" id="discord-widget"></div>
    </div>
  </div>

  <div id="fallback-discord" class="alert alert-info text-center discord-fallback" style="display:none;">
  ...
  (function () {
    ...
    function buildChannelRows(channels) {
      if (!Array.isArray(channels) || channels.length === 0) {
        return '<div class="discord-empty-state">' + escapeHtml(DISCORD_CONFIG.texts.unavailable) + '</div>';
      }
    ...
    function renderWidget(data) {
      ...
      const inviteUrl = data.instant_invite || ('https://discord.com/channels/' + encodeURIComponent(DISCORD_CONFIG.serverID));
      ...
      widget.innerHTML = ''
        ...
        +       '<span class="discord-stat online"><span class="status-dot online"></span>' + escapeHtml(data.presence_count || onlineMembers.length || 0) + ' ' + escapeHtml(DISCORD_CONFIG.texts.online) + '</span>'
        +       '<span class="discord-stat members"><span class="status-dot offline"></span>' + escapeHtml(members.length) + ' ' + escapeHtml(DISCORD_CONFIG.texts.members) + '</span>'
        +     '</div>'
        +   '</div>'
        + '</div>'
        + '<div class="discord-voice-channels">'
        +   '<div class="discord-section-title"><i class="bi bi-headphones me-1"></i>' + escapeHtml(DISCORD_CONFIG.texts.voiceChannels) + '</div>'
        +   buildChannelRows(channels)
        +   '<div class="discord-section-title discord-members-title"><i class="bi bi-people-fill me-1"></i>' + escapeHtml(DISCORD_CONFIG.texts.onlineMembers) + '</div>'
        +   '<div class="discord-members-list">' + buildMemberRows(onlineMembers) + '</div>'
        + '</div>'
        + '<div class="card-footer discord-footer">'
        +   '<a href="' + escapeHtml(inviteUrl) + '" target="_blank" rel="noopener noreferrer" class="btn btn-discord w-100" aria-label="' + escapeHtml(DISCORD_CONFIG.texts.join) + ' - ' + escapeHtml(data.name || 'Discord') + '">'
        +     '<i class="bi bi-discord me-2"></i>'
        +     escapeHtml(DISCORD_CONFIG.texts.join)
        +   '</a>'
        + '</div>';
    ...
    function showUnavailable() {
      const card = document.getElementById('discord-card');
      const widget = document.getElementById('discord-widget');
      const fallback = document.getElementById('fallback-discord');
      if (widget) {
        widget.innerHTML = '<div class="discord-empty-state">' + escapeHtml(DISCORD_CONFIG.texts.unavailable) + '</div>';
      }
      if (card) {
        card.style.display = 'block';
      }
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      if (consent === 'accepted' && DISCORD_CONFIG.serverID) {
