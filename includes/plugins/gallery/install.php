...
$gallerySeedRows = [
    ...
    [7, 'img_6883918321c6a.jpg', 'Bruecke aus Stahl und Dunst', 'Eine industrielle Bruecke verschwindet im fernen Dunst ueber dem Fluss.', 'Stahlbruecke ueber Fluss im hellen Morgen­dunst', 'bruecke,fluss,stahl,dunst', 'Tarin Cole', ''],
...
foreach ($gallerySeedRows as $row) {
    [$id, $filename, $title, $caption, $altText, $tags, $photographer, $class] = $row;
    safe_query("
        INSERT IGNORE INTO plugins_gallery
            (id, filename, title, caption, alt_text, tags, photographer, class, upload_date, position, category_id)
        VALUES
            ({$id}, '{$filename}', '{$title}', '{$caption}', '{$altText}', '{$tags}', '{$photographer}', '{$class}', CURRENT_TIMESTAMP, {$id}, 1)
    ");
}
