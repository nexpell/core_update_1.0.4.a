<?php
// Failsafe: ensure admin log table exists before any reads.
...
safe_query("INSERT IGNORE INTO `plugins_achievements` (`id`, `category_id`, `name`, `description`, `type`, `trigger_value`, `trigger_condition`, `image`, `is_standalone`, `sort_order`, `show_in_overview`, `allow_html`) VALUES
...
// Wir prüfen erst, ob das Plugin schon existiert, um doppelte Inserts sauber zu handhaben
$check = safe_query("SELECT modulname FROM settings_plugins WHERE modulname = 'achievements'");
if (mysqli_num_rows($check) == 0) {
    safe_query("INSERT IGNORE INTO settings_plugins 
        (modulname, admin_file, activate, author, website, index_link, version, path, status_display, plugin_display, widget_display, delete_display, sidebar) 
    VALUES
        ('achievements', 'admin_achievements', 1, 'Fjolnd', 'https://www.nexpell.de', 'achievements', '1.0', 'includes/plugins/achievements/', 1, 1, 1, 1, 'activated')
    ");
} else {
    // Update, falls es schon da ist (Repair-Modus)
    safe_query("UPDATE settings_plugins SET 
        admin_file = 'admin_achievements',
        path = 'includes/plugins/achievements/',
        version = '1.0'
        WHERE modulname = 'achievements'
    ");
}


safe_query("INSERT IGNORE INTO `settings_plugins_lang` 
    (`content_key`, `language`, `content`, `updated_at`) 
VALUES
    -- NAME
    ('plugin_name_achievements', 'de', 'Achievements', NOW()),
    ('plugin_name_achievements', 'en', 'Achievements', NOW()),
    ('plugin_name_achievements', 'it', 'Achievements', NOW()),

    -- INFO
    ('plugin_info_achievements', 'de', 'Das Achievements Plugin erweitert Nexpell um ein flexibles Belohnungs- und Auszeichnungssystem.
Benutzer erhalten automatisch oder manuell vergebene Errungenschaften (Badges) für bestimmte Aktivitäten, Meilensteine oder besondere Leistungen.', NOW()),

    ('plugin_info_achievements', 'en', 'The Achievements plugin extends Nexpell with a flexible reward and recognition system.
Users receive automatically or manually assigned achievements (badges) for specific activities, milestones, or special accomplishments.', NOW()),

    ('plugin_info_achievements', 'it', 'Il plugin Achievements estende Nexpell con un sistema flessibile di premi e riconoscimenti.
Gli utenti ricevono automaticamente o manualmente risultati (badge) per attività specifiche, traguardi o prestazioni speciali.', NOW());
");
...
safe_query("
    ...
    VALUES
        (3, 'achievements', 'admincenter.php?site=admin_achievements', 1)
");
...
safe_query("
    INSERT IGNORE INTO `navigation_dashboard_lang`
        (`content_key`, `language`, `content`, `updated_at`)
    VALUES
        ('nav_link_{$linkID}', 'de', 'Errungenschaften', NOW()),
        ('nav_link_{$linkID}', 'en', 'Achievements', NOW()),
        ('nav_link_{$linkID}', 'it', 'Risultati', NOW())
");
...
safe_query("
    ...
    VALUES
        (3, 'achievements', 'index.php?site=achievements', 1, 1, NOW())
");
...
safe_query("
    INSERT IGNORE INTO `navigation_website_lang`
        (`content_key`, `language`, `content`, `updated_at`)
    VALUES
        ('nav_sub_{$snavID}', 'de', 'Errungenschaften', NOW()),
        ('nav_sub_{$snavID}', 'en', 'Achievements', NOW()),
        ('nav_sub_{$snavID}', 'it', 'Risultati', NOW())
");
