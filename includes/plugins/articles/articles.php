...
$lang = strtolower((string)($languageService->detectLanguage() ?: ($_SESSION['language'] ?? 'de')));
...
if ($action == "show" && isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if (mysqli_num_rows($ergebnis)) {
        ...
        // Pagination
        if ($total_pages > 1) {
            $baseUrl = SeoUrlHandler::convertToSeoUrl(
                'index.php?site=articles&action=show&id=' . intval($category_id)
            );
...
if ($action == "watch" && isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if (mysqli_num_rows($articleQuery)) {
        ...
        if (!empty($article['slug'])) {
            $url  = htmlspecialchars($article['slug'], ENT_QUOTES, 'UTF-8');

            $link =
                '<div class="article-link">' .
                    htmlspecialchars($languageService->get('link'), ENT_QUOTES, 'UTF-8') . ': ' .
        ...
        if ($article['allow_comments']) {
            ...
            while ($row = mysqli_fetch_array($comments)) {
                ...
                // Avatar + Username für den Kommentarautor
                    ...
                    $commentProfileUrl = SeoUrlHandler::convertToSeoUrl(
                        'index.php?site=profile&userID=' . $commentUserID
                    );
                    $username = '<a href="' . htmlspecialchars($commentProfileUrl) . '">
                ...
                echo '<li class="list-group-item border-bottom">
                    <div class="d-flex mt-4">
                        <div class="flex-shrink-0">
                            <a href="' . htmlspecialchars($commentProfileUrl) . '">
            ...
            if ($loggedin) {
                echo '<form method="POST" action="index.php?site=articles&action=watch&id=' . $id . '" class="mt-4">
                    ...
                    <button type="submit" name="submit_comment" class="btn btn-primary mt-3">Kommentar abschicken</button>
                </form>';
            } else {
    ...
    if (mysqli_num_rows($cats_result) > 0) {
        ...
        if ($total_pages > 1) {
            $baseUrl = SeoUrlHandler::convertToSeoUrl('index.php?site=articles');
            $baseUrl = rtrim($baseUrl, '/') . '/';
            echo $tpl->renderPagination($baseUrl, (int)$page, (int)$total_pages);
        }
    } else {
